...
<p><strong><a href="https://labuladong.online/algo/dynamic-programming/freedom-trail/" target="_blank">⭐️labuladong 题解</a></strong></p>
...
// 计算圆盘指针在 ring[i]，输入 key[j..] 的最少操作数
func dp(ring string, i int, key string, j int, charToIndex map[rune][]int, memo [][]int) int {
    // base case 完成输入
    if j == len(key) {
        return 0
    }
    ...
    n := len(ring)
    // 做选择
    res := int(^uint(0) >> 1) // Initialize res to the maximum int value
    // ring 上可能有多个字符 key[j]
...
var findRotateSteps = function(ring, key) {
    ...
    for (let i = 0; i < ring.length; i++) {
        ...
        charToIndex[c].push(i);
    }

    // 备忘录全部初始化为 0
    for (let i = 0; i < ring.length; i++) {
        memo[i] = Array(key.length).fill(0);
    }
...
<hr /><details open hint-container details><summary style="font-size: medium"><strong>🎃🎃 算法可视化 🎃🎃</strong></summary><div id="data_freedom-trail"  category="leetcode" ></div><div class="resizable aspect-ratio-container" style="height: 100%;">
